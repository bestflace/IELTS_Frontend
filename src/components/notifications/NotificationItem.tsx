...
function getNotificationHref(notification: NotificationLike) {
  ...
  if (data?.kind === "ATTEMPT_COMMENT" && data?.submissionId) {
    return `/teacher/submissions/${data.submissionId}`;
  }

  if (data?.kind === "ATTEMPT_COMMENT" && data?.attemptId) {
    return `/teacher/submissions`;
  }

  if (data?.kind === "ATTEMPT_COMMENT_REPLY" && data?.attemptId) {
    return `/learner/attempts/${data.attemptId}/review`;
  }
