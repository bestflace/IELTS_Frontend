...
import { ListeningQuestionForm } from "@/components/content-bank/listening/ListeningQuestionForm";
import {
...
export function ListeningQuestionManager({
  listeningSetId,
  questions,
  isPublished = false,
  onChanged,
  ...
  const handleSubmit = async (payload: any) => {
    ...
    try {
      const listeningPayload: ListeningQuestionInput = {
        ...payload,
        sectionLabel: payload.sectionLabel ?? null,
      };

      if (editingQuestion?.id) {
        await updateListeningQuestion(editingQuestion.id, listeningPayload);
      } else {
        await addListeningQuestion(listeningSetId, listeningPayload);
      }
  ...
  return (
    <Card className="relative overflow-hidden rounded-[36px] border border-white/70 bg-white/80 shadow-[0_30px_90px_rgba(14,165,233,0.12)] backdrop-blur-2xl">
      ...
      <CardContent className="space-y-4">
        ...
        {isPublished ? (
          <div className="rounded-2xl border border-cyan-100 bg-amber-50 p-4 text-sm leading-6 text-amber-700">
            Listening Set này đang được xuất bản. Nếu backend không cho sửa câu
        ...
        {sortedQuestions.length ? (
          ...
          <EmptyState
            ...
            description="Thêm câu hỏi để Listening Set có thể được xuất bản và sử dụng trong đề thi."
            action={
              <Button onClick={openCreateForm} disabled={isPublished}>
                <Plus className="h-4 w-4" />
      ...
      <Modal
        ...
        title={
          editingQuestion ? "Sửa câu hỏi Listening" : "Thêm câu hỏi Listening"
        }
      >
      ...
      <ConfirmDialog
        ...
        title="Xóa câu hỏi?"
        description="Câu hỏi này sẽ bị xóa khỏi Listening Set."
