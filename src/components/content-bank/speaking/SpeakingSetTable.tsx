...
import {
  deleteSpeaking,
  getAdminSpeakingList,
  publishSpeaking,
  unpublishSpeaking,
} from "@/lib/api/speaking.api";
import { getErrorMessage } from "@/lib/api/client";
import type { PublishStatus, SpeakingSet } from "@/types";
...
export function SpeakingSetTable() {
  const [items, setItems] = useState<SpeakingSet[]>([]);
  ...
  const [deleteTarget, setDeleteTarget] = useState<SpeakingSet | null>(null);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const res = await getAdminSpeakingList({
  ...
  const handlePublish = async (item: SpeakingSet) => {
    setActionLoading(true);
    setError("");

    try {
      if (item.status === "PUBLISHED") {
        await unpublishSpeaking(item.id);
      } else {
        await publishSpeaking(item.id);
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteSpeaking(deleteTarget.id);
  ...
  if (loading) {
    return <LoadingState label="Đang tải ngân hàng Speaking..." />;
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <h2 className="mt-1 font-serif text-2xl font-black text-slate-950">
                Danh sách Speaking Set
              </h2>
              <p className="mt-1 text-sm text-slate-500">
                Chỉ những Speaking Set đã xuất bản mới được chọn khi tạo đề thi.
              </p>
            </div>
        ...
        <CardContent>
          <div className="mb-5 grid gap-3 lg:grid-cols-[1fr_220px]">
            <div className="relative">
              ...
              <Input
                ...
                className="pl-9"
                placeholder="Tìm theo topic hoặc tiêu đề..."
              />
            </div>
          ...
          {filteredItems.length ? (
                      ...
                      >
                        <td className="px-4 py-4 align-top">
                          <p className="font-semibold text-slate-950">
                            {item.title || item.topic || "Speaking Set"}
                          </p>
                        </td>
                        ...
                        <td className="px-4 py-4 align-top">
                          <div className="flex justify-end gap-2">
                            <Link href={`/admin/speaking-sets/${item.id}`}>
            ...
            <EmptyState
              title="Chưa có Speaking Set"
              description="Tạo set đầu tiên hoặc import CSV để xây dựng ngân hàng Speaking."
              action={
                <Link href="/admin/speaking-sets/new">
                  <Button>Tạo Speaking Set</Button>
                </Link>
              }
            />
          )}
      ...
      <ConfirmDialog
        ...
        title="Xóa Speaking Set?"
        description="Speaking Set này sẽ bị xóa khỏi ngân hàng đề. Không nên xóa nếu nội dung đã được dùng trong đề thi."
